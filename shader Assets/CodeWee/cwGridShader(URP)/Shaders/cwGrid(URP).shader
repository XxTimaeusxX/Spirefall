	...
	SubShader
	{
		...
		v2f vert (appdata v)
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				// Calculate base color + emission based on grid lines
				float3 rgb = _sc.rgb * sa +	_Color.rgb * ma;
				float3 emission = _EmissionColor.rgb * _EmissionStrength * (ma + sa);
				
				float a = sa + ma; 
				float4 pass2 = float4(rgb + emission, a);
